...
const EditarTrabajador = ()=>{
    ...
    const handleCreateTrabajador = () => {
        ...
        axios.put(`${useHost()}/editartrabajador`, {
           id, 
           nombre,
           turno, 
           libre
    ...
    const getDatosTrabajador = () => {
        axios.get(`${useHost()}/trabajador/${id}`)
            .then((response) => {
                const resultado = response.data[0];
                if (resultado && resultado.length > 0 && Array.isArray(resultado)) {
                    resultado.forEach((trabajador) => {
                        setNombre(trabajador.Nombre);
                        setTurno(trabajador.Turno);
                        setLibre(trabajador.Libre);
                    });
                }
            })
            .catch((error) => {
    ...
    return(
        <View>
            ...
            <Text style={styles.titleText}>Editar Trabajador</Text>
            ...
            <View style={styles.button}>
                <Pressable style={styles.pressableButton} onPress={handleCreateTrabajador}>
                    <Text style={styles.pressableText}>Editar Trabajador</Text>
                </Pressable> 
            </View>
